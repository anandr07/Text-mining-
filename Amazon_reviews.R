...
aurl <-  "https://www.amazon.in/Apple-iPhone-11-64GB-Green/product-reviews/B07XVKBY68/ref=cm_cr_arp_d_viewopt_srt?reviewerType=all_reviews&pageNumber=1&sortBy=helpful"
amazon_reviews <- NULL
for (i in 1:10){
  murl <- read_html(as.character(paste(aurl,i,sep="")))
  rev <- murl %>%
    html_nodes(".a-size-base.review-text.review-text-content") %>%
    html_text()
  amazon_reviews <- c(amazon_reviews,rev)
}
write.table(amazon_reviews,"iphone11.txt",row.name=F)
getwd()

View(amazon_reviews)

#Removing similar reviews
reviews_df=unique(amazon_reviews)
View(reviews_df)
